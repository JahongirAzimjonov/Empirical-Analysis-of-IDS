...
def rfe_with_cv(X,y, plot_path):
    min_features_to_select = 1  # Minimum number of features to consider
    # clf = lm.LogisticRegression()
    classifiers = [lm.LogisticRegression(penalty='l2', 
                                random_state=global_random_state_number,
                                max_iter=100,
                                tol=0.00001791,
                                C = 0.00091                                    
                                ),
                   lm.RidgeClassifier(random_state=global_random_state_number),
                   lm.SGDClassifier(random_state=global_random_state_number)
                  ]
    clf = classifiers[2]
    ...
    plt.figure(figsize=(31,19), layout="constrained")
    ...
    plt.xticks(ha='right', fontsize=29, fontweight="bold")
    plt.yticks(fontsize=29, fontweight="bold")
    
    # plt.title(f"Recursive Feature Elimination with correlated features.\nOptimal number of features: {rfecv.n_features_}")
    plt.title(f"  Relationship between classification accuracy and number of features. \
              \nOptimal number of trainable features is {rfecv.n_features_}.",
              fontdict={'fontsize':43, 'fontweight':"bold"})
    ...
    plt.savefig(plot_path + "NumOfFeatures_ClassAccuracy.png", dpi = 100)
    plt.show()
...
def botiot18_format_fit(df):
    df = df.drop(df.columns[0], axis = 1)
    df = df.drop(['category', 'subcategory'], axis = 1)
    return df

def nbaiot21_format_fit(df):
    df = df.drop(['sublabels', 'labels'], axis = 1)
    return df

def cicids17_format_fit(df):
    df = df.drop(['Label'], axis = 1)
    return df

def insdn20_format_fit(df):
    df = df.drop(['FlowID','Label'], axis = 1)
    return df

def nslkdd21_format_fit(df):
...
def dataset_imbalance_test(target_output, dataset_text, path_to_save_plots, plot_name):
    ...
    col_normal = 'lime'
    col_attack = 'magenta'
    if classes[0] == 0.0 or classes[0] == 0 or classes[0] == 'Normal' or classes[0] == 'normal':
        classes = ["Normal", "Attack"]
        plt.bar(classes, class_distribution, color=(col_normal, col_attack))
    if classes[0] == 1.0 or classes[0] == 1 or classes[0] == 'Attack' or classes[0] == 'attack':
        classes = ["Attack", "Normal"]
        plt.bar(classes, class_distribution, color=(col_attack, col_normal))
...
def test_ml_model_with_selected_features(model, data_modeled, y, selected_features, fs_method_name, fs_model_name, path_to_save_plots, path_to_save_docs):
    ...
    plt.savefig(f"{path_to_save_plots}{fs_method_name}_{fs_model_name}.png", dpi=300)
...
ml_models = [
                ...
                #          random_state=global_random_state_number, 
                #          max_iter=500), 
                svm.LinearSVC(random_state=global_random_state_number, max_iter=100),
                ...
                #         max_iter=100, 
                #         random_state=global_random_state_number),
                svm.SVC(random_state=global_random_state_number, max_iter=100),
                ...
                #             random_state=global_random_state_number)
                lm.SGDClassifier(random_state=global_random_state_number, max_iter=111)
            ]
...
fs_models = [   
                lm.Lasso(random_state=global_random_state_number, max_iter=111),
                lm.LinearRegression(),
                lm.LogisticRegression(random_state=global_random_state_number, max_iter=100),
...
def app():
    ...
    main_res_path = "Revised/Results/"

    main_plot_dir = f"{main_res_path}{dataset_file_name}/common_plots/"
    if not os.path.exists(main_plot_dir):
        os.makedirs(main_plot_dir)
    main_doc_dir = f"{main_res_path}{dataset_file_name}/common_docs/"
    ...
    if dataset_file_name in ["kddcup99-corrected", "kddcup99-data_10_percent", "kddcup99-data"]:
        df = kddcup99_format_fit(df)
    
    elif dataset_file_name in ["botiot18-5000", "botiot18"]:
        df = botiot18_format_fit(df)
    
    elif dataset_file_name in ["nbaiot21"]:
        df = nbaiot21_format_fit(df)
    
    elif dataset_file_name in ["cicids17"]:
        df = cicids17_format_fit(df)
    
    elif dataset_file_name in ["insdn20"]:
        df = insdn20_format_fit(df)

    elif dataset_file_name == "nslkdd21":
        df = nslkdd21_format_fit(df)
    ...
    dataset_imbalance_test(target_output=df['attack'], dataset_text=dataset_text, path_to_save_plots=main_plot_dir, plot_name="before")
    ...
    for ml_ID in range(0, len(ml_model_names)):
        model_all_features_plot_dir = f"{main_res_path}{dataset_file_name}/{ml_model_names[ml_ID]}/"
        if not os.path.exists(model_all_features_plot_dir):
            os.makedirs(model_all_features_plot_dir)
        model_all_features_doc_dir = f"{main_res_path}{dataset_file_name}/{ml_model_names[ml_ID]}/"
        ...
        for fs_ID in range(0, len(fs_model_names)):
            ...
            plot_dir = f"{main_res_path}{dataset_file_name}/{ml_model_names[ml_ID]}/{fs_model_names[fs_ID]}/plots/"
            if not os.path.exists(plot_dir):
                os.makedirs(plot_dir)
            doc_dir = f"{main_res_path}{dataset_file_name}/{ml_model_names[ml_ID]}/{fs_model_names[fs_ID]}/docs/"
